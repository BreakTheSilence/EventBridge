...
export class ParticipantsClient {
    ...
    getParticipantsNotPresentInEventByEventId(eventId: number): Promise<ParticipantDto[]> {
        let url_ = this.baseUrl + "/api/Participants/event/{eventId}";
        if (eventId === undefined || eventId === null)
            throw new Error("The parameter 'eventId' must be defined.");
        url_ = url_.replace("{eventId}", encodeURIComponent("" + eventId));
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetParticipantsNotPresentInEventByEventId(_response);
        });
    }

    protected processGetParticipantsNotPresentInEventByEventId(response: Response): Promise<ParticipantDto[]> {
        followIfLoginRedirect(response);
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
